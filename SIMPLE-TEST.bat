@echo off
REM PatentFlow Enterprise - SIMPLE WORKING TEST
REM This will create a minimal working version
...
echo               ^<p^><strong^>User:^</strong^> {session.user?.email || 'Unknown'}^</p^> >> src\app\dashboard\simple-page.tsx
echo               ^<p^><strong^>Name:^</strong^> {session.user?.name || 'Unknown'}^</p^> >> src\app\dashboard\simple-page.tsx
echo               ^<p^><strong^>Role:^</strong^> {session.user?.role || 'Unknown'}^</p^> >> src\app\dashboard\simple-page.tsx
